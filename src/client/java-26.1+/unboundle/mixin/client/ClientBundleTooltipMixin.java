...
public class ClientBundleTooltipMixin {
    ...
    @Shadow
    private static int getContentXOffset(int i) { return 0; }
    ...
    @WrapMethod(method = "extractBundleWithItemsTooltip(Lnet/minecraft/client/gui/Font;IIIILnet/minecraft/client/gui/GuiGraphics;Lorg/apache/commons/lang3/math/Fraction;)V")
    private void addTopLeftCounter(Font font, int x, int y, int w, int h, GuiGraphics guiGraphics, Fraction weight, Operation<Void> original) {
...
//                this.contents.getNumberOfItemsToShow(), itemsToShowStart, itemsToShowEnd, BundleTooltipContext.rowOffset);
        ...
        // h is unused.
        int startX = x + getContentXOffset(w) + gridWidth;
        ...
        for (int row = 1; row <= this.gridSizeY(); row++) {
            for (int col = 1; col <= UnboundleConfig.config().columns; col++) {
                ...
                if (shouldRenderSurplusText(hiddenBelow, col, row)) {
                    extractCount(loopX, loopY, this.getAmountOfHiddenItems(list), font, guiGraphics);
                }
                // Checks if we're at the top left slot, and if we need to draw a counter there.
                    ...
                    extractCount(loopX, loopY, this.getAmountOfHiddenItemsForTopLeft(), font, guiGraphics);
                }
                // Otherwise, draw item if we're not at the end of the list already
                else if (shouldRenderItemSlot(list, slotIndex)){
                    this.extractSlot(slotIndex, loopX, loopY, list, slotIndex, font, guiGraphics);
                    slotIndex++;
                }
            }
        }
        ...
        extractProgressbar(x + getContentXOffset(w), y + this.itemGridHeight() + SLOT_MARGIN, font, guiGraphics, weight);
    }

    @Shadow
    private List<ItemStackTemplate> getShownItems(int numberOfItemsToShow) { return null; }
...
//                    numberOfItemsToShow, itemsToShowStart, itemsToShowEnd);
    ...
    @Shadow
    private static boolean shouldRenderItemSlot(List<ItemStackTemplate> list, int i) { return false; }

    @Shadow
    private int getAmountOfHiddenItems(List<ItemStackTemplate> list) { return 0; }
    ...
    @Shadow
    private void extractSlot(final int slotNumber, final int drawX, final int drawY, final List<ItemStackTemplate> shownItems, final int slotIndex, final Font font, final GuiGraphics graphics) {}
    ...
    @WrapMethod(method = "extractSlot(IIILjava/util/List;ILnet/minecraft/client/gui/Font;Lnet/minecraft/client/gui/GuiGraphics;)V")
    private void addShadows(int slotIndex, int j, int k, List<ItemStack> list, int slotIndex2, Font font, GuiGraphics guiGraphics, Operation<Void> original) {
        ...
        boolean isSelected = topLeftSlotIndex + itemsToShowStart == this.contents.getSelectedItemIndex();
        ...
        Fraction weight = ((BundleContentsAccessor)(Object) contents).invokeGetWeight(itemStack).getOrThrow();
        ...
        if (isSelected) { // if the item was selected
            ...
            guiGraphics.item(itemStack, j + SLOT_MARGIN, k + SLOT_MARGIN, slotIndex2);
            guiGraphics.itemDecorations(font, itemStack, j + SLOT_MARGIN, k + SLOT_MARGIN);
    ...
    @Shadow
    private static void extractCount(int i, int j, int count, Font font, GuiGraphics guiGraphics) {}

    @Shadow
    private void extractSelectedItemTooltip(Font font, GuiGraphics guiGraphics, int i, int j, int k) {}

    @Shadow
    private static void extractProgressbar(int i, int j, Font font, GuiGraphics guiGraphics, Fraction weight) {}
    ...
    private static int extractProgressbar$BlitSprite2$dynamicProgressBarWidth(int ninetySix) {
    ...
    @ModifyArg(
            ...
            at = @At(
                    value = "INVOKE",
                    target = "Lnet/minecraft/client/gui/GuiGraphics;centeredText(Lnet/minecraft/client/gui/Font;Lnet/minecraft/network/chat/Component;III)V",
                    ordinal = 0
            ),
            index = 2
    )
    private static int extractProgressbar$CenteredString$dynamicI(int i) {
    ...
    @ModifyConstant(method = "extractEmptyBundleDescriptionText(IILnet/minecraft/client/gui/Font;Lnet/minecraft/client/gui/GuiGraphics;)V", constant = @Constant(intValue = 96))
    private static int extractEmptyBundleDescriptionText$dynamicProgressBarWidth(int original) {
        return progressBarWidth;
    }
    ...
    @ModifyConstant(method = "getProgressBarFill(Lorg/apache/commons/lang3/math/Fraction;)I", constant = @Constant(intValue = 94))
    ...
    @Inject(
            method = "getProgressBarFillText(Lorg/apache/commons/lang3/math/Fraction;)Lnet/minecraft/network/chat/Component;",
            at = @At("RETURN"),
            cancellable = true
    )
    private static void getProgressBarFillText$unboundle(Fraction weight, CallbackInfoReturnable<Component> cir) {
