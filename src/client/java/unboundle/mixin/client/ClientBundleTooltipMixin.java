...
public class ClientBundleTooltipMixin {
    ...
    public int dynamicWidth(int original) {
        ...
        } else if (size < BundleContext.config().columns) {
            setTooltipWidth(Math.max(size, 4) * SLOT_SIZE);
        } else {
            setTooltipWidth(BundleContext.config().columns * SLOT_SIZE);
        }
        return gridWidth;
    }
    ...
    private static int dynamicColumns(int original) {
        return BundleContext.config().columns;
    }
    ...
    private static int dynamicMaxSlots(int original) {
        return BundleContext.config().maxSlots();
    }
    ...
    private void addTopLeftCounter(Font font, int i, int j, int k, int l, GuiGraphics guiGraphics, Operation<Void> original) {
        ...
        int itemsToShowStart = BundleContext.getItemsToShowStart(this.contents.size());
        int itemsToShowEnd = BundleContext.getItemsToShowEnd(this.contents.size(), this.contents.getNumberOfItemsToShow());
...
//                this.contents.getNumberOfItemsToShow(), itemsToShowStart, itemsToShowEnd, BundleContext.rowOffset);
        ...
        for (int row = 1; row <= this.gridSizeY(); row++) {
            for (int col = 1; col <= BundleContext.config().columns; col++) {
    ...
    private List<ItemStack> dynamicItemWindows(int numberOfItemsToShow, Operation<List<ItemStack>> original) {
        int itemsToShowStart = BundleContext.getItemsToShowStart(this.contents.size());
        int itemsToShowEnd = BundleContext.getItemsToShowEnd(this.contents.size(), numberOfItemsToShow);
...
//                    numberOfItemsToShow, itemsToShowStart, itemsToShowEnd);
    ...
    private boolean shouldRenderSurplusTextTopLeft(boolean bl, int i, int j) {
        return bl && i == BundleContext.config().columns && j == this.gridSizeY();
    }
    ...
    private int getAmountOfHiddenItems$considerRowScrolls(int original) {
        return this.contents.itemCopyStream()
                .skip(BundleContext.getItemsToShowEnd(this.contents.size(), this.contents.getNumberOfItemsToShow()) + 1)
    ...
    private int getAmountOfHiddenItemsForTopLeft() {
        return this.contents.itemCopyStream()
                .limit(BundleContext.getItemsToShowStart(this.contents.size()))
                .mapToInt(ItemStack::getCount)
                .sum();
    }
    ...
    private void addShadows(int slotIndex, int j, int k, List<ItemStack> list, int slotIndex2, Font font, GuiGraphics guiGraphics, Operation<Void> original) {
        ...
        int shadowTint = BundleContext.highContrast ? 0xFFFFFFFF : 0xFF000000;
        Fraction weight = BundleContext.getWeight(itemStack);
